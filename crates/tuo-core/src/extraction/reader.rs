...
use tuo_shared::errors::tuo::TuoError;
...
pub trait UniReaderTrait: Sync + Send {
    ...
    async fn read(&self, file_path: &str) -> TuoResult<ParsedDocument> {
        ...
        match reader_providers {
            None => {
                Err(TuoCoreError::UniReaderHasNoReaderProvider)?
            }
...
pub trait UniFolderReaderTrait: Sync + Send {
    ...
    async fn read_folder(&self, directory_path: &str) -> TuoResult<Vec<ParsedDocument>> {
