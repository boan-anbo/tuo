...
pub trait ProjectAssistantTrait: AgentTrait {
    ...
    async fn run_project(&self, project: Project) -> TuoResult<ProjectResult>;
    async fn run_prep_stage(&self, project: Project) -> TuoResult<Project>;

    async fn run_iterating_stage(&self, project: Project) -> TuoResult<Project>;
    async fn run_conclusion_stage(&self, project: Project) -> TuoResult<Project>;
}
...
impl ProjectAssistantTrait for ProjectAssistant {
    ...
    async fn run_prep_stage(&self, mut project: Project) -> TuoResult<Project> {
        ...
        if !is_ready {
            ...
            Err(TuoCoreError::GenericError("Director is not ready to iterate".to_string()))?
        } else {
    ...
    async fn run_conclusion_stage(&self, mut project: Project) -> TuoResult<Project> {
        let director = project.director.clone();
        let assistant = project.assistant.clone();
